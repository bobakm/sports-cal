...
/** Domestic cups. ESPN slugs verified: 'eng.fa' = FA Cup, 'eng.league_cup' =
 *  Carabao, 'eng.charity' = Community Shield. ('eng.trophy' is the EFL Trophy,
 *  which is U21 sides — deliberately excluded.) */
const CUPS = ['eng.fa', 'eng.league_cup', 'eng.charity'];
const EPL = (slug: string, name: string, short: string, id: string): Team => ({
  ...
  sources: [{ sport: 'soccer', league: 'eng.1', teamId: id },
            ...[...EURO, ...CUPS].map(league => ({ sport: 'soccer', league, teamId: id }))],
});
...
export const TEAMS: Team[] = [
  ...
  NCAAF('texas-am',   'Texas A&M Aggies',  'TAM', '245'),
  NCAAF('penn-state', 'Penn State Nittany Lions', 'PSU', '213'),
  ...
  NATIONAL('usa-men',     'USA National (Men)',   'USA', '660',
    ['fifa.world', 'fifa.friendly', 'concacaf.gold', 'fifa.worldq.concacaf']),
  NATIONAL('usa-women',   'USA National (Women)', 'USW', '2765',
  ...
  NATIONAL('netherlands', 'Netherlands National', 'NED', '449', UEFA_NAT),
];
